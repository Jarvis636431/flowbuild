...
function parseDependencies(value: string | null | undefined): string[] {
  ...
  if (typeof value === 'string') {
    return value.split(',').map(dep => dep.trim()).filter(dep => dep && dep !== '无');
  }
  return [];
}
 ...
 * 从Excel文件读取项目数据
 */
export async function readProjectFromExcel(fileName: string): Promise<Project | null> {
  try {
    ...
    dataRows.forEach((row, rowIndex) => {
      ...
      headers.forEach((header, colIndex) => {
        ...
        if (mappedField && row[colIndex] !== undefined && row[colIndex] !== '') {
